...
function validateDeepLink(url) {
  ...
  try {
    ...
    // Security (H3 / FIX 4): The 'token' param in the deep link is a short-lived
    // one-time download token generated by /api/external-download-token on the server.
    // It is NOT the long-lived session bearer — the Companion uses its own persisted
    // sessionToken for authenticated API calls (e.g. resolve-download-token).
    // Validate the one-time token format: alphanumeric + dashes/underscores, under 200 chars.
