...
// Security (L4): Public key is hardcoded as a constant to prevent tampering via the
// on-disk PEM file (which could be replaced if the asar is modified or the assets
// directory is writable). The env-var override is kept for development/rotation only.
const UPDATE_ED25519_PUBLIC_KEY_PINNED = `-----BEGIN PUBLIC KEY-----
MCowBQYDK2VwAyEAGn4Wwasnb73HzzQLzGXZt+QsfhTPnzLm6m37wAAF8RA=
-----END PUBLIC KEY-----`;

function getUpdateEd25519PublicKeyPem() {
  // Allow env-var override for key rotation during development/testing
  const v = process.env.ARMGDDN_UPDATE_ED25519_PUBKEY_PEM;
  if (v && typeof v === 'string' && v.trim()) {
    return v.trim();
  }

  // Use the pinned constant — do not read from disk at runtime (tamper-resistant)
  return UPDATE_ED25519_PUBLIC_KEY_PINNED;
}
      ...
      res.on('end', async () => {
        try {
          ...
          if (json.redirect && json.newRemote && json.newPath) {
            // Security (M3): validate server-supplied redirect params before use
            if (typeof json.newRemote !== 'string' || json.newRemote.length > 4096 ||
                typeof json.newPath !== 'string' || json.newPath.length > 4096) {
              reject(new Error('Invalid redirect parameters from server'));
              return;
            }
...
// Security (M4): Rotate debug.log if it exceeds 10 MB to prevent unbounded disk growth.
// Keeps at most 2 log files: debug.log and debug.log.1.
function rotateLogIfNeeded(logPath) {
  ...
  } catch (e) { /* file doesn't exist yet or stat failed — no rotation needed */ }
}

// Track whether startup rotation has run to avoid checking on every write
let _logRotationDoneAtStartup = false;
...
function logToFile(message) {
  try {
    ...
    // Rotate at startup (once) and on each write if already over limit
    if (!_logRotationDoneAtStartup) {
      _logRotationDoneAtStartup = true;
...
function validateDeepLink(url) {
  ...
  try {
    ...
    // The 'token' param in the deep link IS a real app-session bearer token — the
    // browser mints it fresh (via /api/generate-app-token) right before building the
    // link, since it's already authenticated. resolve-download-token uses it as a
    // one-shot attempt only; it is never persisted as the Companion's saved session,
    // so a stale/crafted deep link can't silently replace what the user is logged in as.
    // Validate the one-time download token format: alphanumeric + dashes/underscores, under 200 chars.
    const downloadToken = parsed.searchParams.get('downloadToken');
    if (downloadToken !== null) {
      if (typeof downloadToken !== 'string' ||
          downloadToken.length < 8 ||
          downloadToken.length > 200 ||
          !/^[A-Za-z0-9_-]+$/.test(downloadToken)) {
        logToFile('Deep link rejected - invalid downloadToken format');
        return null;
      }
    }
...
async function verifySession() {
  ...
  return new Promise((resolve) => {
    const makeRequest = (url) => {
      ...
      const req = https.request(options, (res) => {
        // Follow redirects — Security (M2): validate redirect host against allowlist
        if (res.statusCode === 301 || res.statusCode === 302) {
          ...
          if (location) {
            ...
            try {
              ...
              if (!isAllowedServiceHost(redirectHostname)) {
                logToFile(`verifySession: redirect to disallowed host blocked: ${redirectHostname}`);
                return resolve(false);
              }
            } catch (e) {
              return resolve(false);
...
// Security (L2): Session tokens are generated by server.js as crypto.randomBytes(32).toString('hex')
// — a 64-character lowercase hex string. We enforce this exact format.
function isValidToken(token) {
  if (typeof token !== 'string') return false;
  // 64-char lowercase hex (32 random bytes)
  if (/^[0-9a-f]{64}$/.test(token)) return true;
  // Fallback: accept any reasonable-length alphanumeric token in case the format changes
  if (token.length >= 10 && token.length <= 500 && /^[A-Za-z0-9_\-+/=.]+$/.test(token)) return true;
  return false;
}
...
// Single attempt to resolve a download token against the API using a given bearer
// token. Never throws — resolves { ok: true, json } or { ok: false, statusCode, errMsg }
// so callers can decide whether to fall back rather than treating every failure as fatal.
function attemptResolveDownloadToken(downloadToken, bearerToken) {
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          resolve({ ok: false, statusCode: res.statusCode, errMsg: 'Invalid JSON response' });
        }
      });
    });
...
ipcMain.handle('resolve-download-token', async (event, downloadToken, urlToken) => {
  ...
  // The browser mints a fresh app token and hands it to us on every download deep
  // link (it's already authenticated when the user clicks download). Try it first,
  // as a ONE-SHOT attempt only — never persisted as our saved session — so a stale or
  // malicious deep link can't silently replace what the user is actually logged in as.
  // The server independently enforces that this token's user matches the downloadToken's
  // owner (403 otherwise), so this can't be used to hijack someone else's download.
  if (isValidToken(urlToken)) {
    const attempt = await attemptResolveDownloadToken(downloadToken, urlToken);
    if (attempt.ok) {
      return attempt.json;
    }
  ...
  // Fall back to the Companion's own persisted session token.
  ...
  if (result.statusCode === 401 || /auth/i.test(result.errMsg)) {
    logToFile('resolve-download-token: server rejected token, clearing session and opening auth window');
    sessionToken = null;
    openAuthWindow().catch(() => {});
    throw new Error('Your session expired. Please log in again and retry the download.');
  }
...
// Security (M1): Hostname allowlist for open-external — protocol check alone is insufficient.
...
ipcMain.handle('open-external', (event, url) => {
  // Security: Only allow HTTPS URLs to allowlisted hostnames
  try {
    if (typeof url !== 'string' || !url) return { success: false, error: 'Invalid URL' };
    const u = new URL(url);
    if (u.protocol !== 'https:' || !u.hostname) return { success: false, error: 'Only HTTPS allowed' };
    if (!ALLOWED_EXTERNAL_HOSTS.has(u.hostname)) {
      logToFile(`open-external rejected - host not allowed: ${u.hostname}`);
...
// Security (C1/H1): installerUrl is NOT returned to the renderer — the install-update
// handler derives the URL internally to prevent renderer-supplied URL attacks.
ipcMain.handle('check-updates', async () => {
  const result = await fetchLatestReleaseInfo();
  // Strip installerUrl — renderer must not supply it back to install-update
  const { installerUrl: _stripped, ...safeResult } = result;
  // Surface whether auto-install is available without exposing the URL
  safeResult.canAutoInstall = !!_stripped;
  return safeResult;
});
...
// Security (C1/H1): The renderer-supplied URL is ignored entirely. The installer URL is
// derived freshly in the main process from the GitHub releases API to prevent renderer
// compromise from redirecting the update to an attacker-controlled URL.
ipcMain.handle('install-update', async (event, _ignoredRendererUrl, options) => {
  ...
  // Derive the installer URL internally — never trust the renderer-supplied value
  let installerUrl;
  try {
    const releaseInfo = await fetchLatestReleaseInfo();
    installerUrl = releaseInfo.installerUrl || null;
    if (!installerUrl) {
      _installUpdateInProgress = false;
      return { success: false, error: 'No installer available for this platform' };
    }
  } catch (e) {
    _installUpdateInProgress = false;
    return { success: false, error: 'Failed to fetch installer URL: ' + (e && e.message ? e.message : String(e)) };
  }

  // Enforce HTTPS + allowlisted host + expected GitHub releases path
  try {
    ...
    // Enforce that the path is a GitHub releases asset path
    const p = u.pathname;
    if (u.hostname === 'github.com' && !p.startsWith('/Nildyanna/armgddn-downloader/releases/')) {
      logToFile(`Update - installer path rejected: ${p}`);
      _installUpdateInProgress = false;
      return { success: false, error: 'Installer URL path not allowed' };
    }
  } catch (e) {
    _installUpdateInProgress = false;
            ...
            withTimeout(downloadSignature(sigUrl), 30000, 'signature download').then(async (sigRes) => {
              ...
              try {
                if (platform === 'win32') {
                  ...
                  // Security (C2/M5): Use PowerShell with single-quoted paths to prevent
                  // batch metacharacter injection. Runner scripts written to userData
                  // (user-owned) instead of temp (world-writable).
                  try {
                    const pid = process.pid;
                    const shouldRelaunch = relaunchAfterInstall;
                    ...
                    const runner = [
                      '# ARMGDDN Companion update runner — auto-generated, do not edit',
                      `$pid_to_wait = ${pid}`,
                      `$installer = ${escapePsPath(filePath)}`,
                      `$appExe = ${escapePsPath(process.execPath)}`,
                      `$logPath = ${escapePsPath(wrapperLogPath)}`,
                      ...
                      'while (Get-Process -Id $pid_to_wait -ErrorAction SilentlyContinue) { Start-Sleep -Milliseconds 500 }',
                      'Add-Content -Path $logPath -Value ("[{0}] parent exited" -f (Get-Date -Format o))',
                      '',
                      '# Run installer',
                      'Set-Content -Path $resultPath -Value (\'{"state":"installing"}\')',
                      'if ($silentArg) {',
                      '  $proc = Start-Process -FilePath $installer -ArgumentList $silentArg -Wait -PassThru',
                      '} else {',
                      '  $proc = Start-Process -FilePath $installer -Wait -PassThru',
                      '}',
                      '$rc = $proc.ExitCode',
                      'Add-Content -Path $logPath -Value ("[{0}] installer finished rc=$rc" -f (Get-Date -Format o))',
                      '',
                      ...
                      '}',
                      'Add-Content -Path $logPath -Value ("[{0}] runner done" -f (Get-Date -Format o))',
                      'Remove-Item -Path $MyInvocation.MyCommand.Path -Force -ErrorAction SilentlyContinue'
                    ].join("\r\n");

                    try {
                      fs.writeFileSync(runnerPath, runner, { encoding: 'utf8' });
                      logToFile(`Update - wrote ps1 runner: ${runnerPath}`);
                  ...
                  if (filePath.endsWith('.AppImage')) {
                    ...
                    if (process.env.APPIMAGE) {
                      ...
                      // Security (H2): Escape all paths inserted into bash using single-quote
                      // wrapping to prevent shell metacharacter injection from APPIMAGE env var.
                      function escapeBashSingleQuote(p) {
                        return "'" + String(p).replace(/'/g, "'\\''") + "'";
                      }
                      ...
                      const scriptContent = [
                        ...
                        `mv ${escapeBashSingleQuote(currentPath)} ${escapeBashSingleQuote(currentPath + '.old')}`,
                        '',
                        '# Move new version to original location',
                        `mv ${escapeBashSingleQuote(filePath)} ${escapeBashSingleQuote(currentPath)}`,
                        `chmod +x ${escapeBashSingleQuote(currentPath)}`,
                        '',
                        '# Launch new version',
                        `${escapeBashSingleQuote(currentPath)} &`,
                        'exit 0'
                      ].join('\n');
